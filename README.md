...
连上校园网 Wi-Fi 后自动完成 Portal 认证，不用每次手动打开浏览器输账号密码。
登录一次配置，之后开机静默认证。
...
| 项 | 填什么 | 说明 |
|---|---|---|
| **账号** | 你的学号 | **只填裸账号**，不要加前缀后缀 |
| **密码** | 你的密码 | 用 DPAPI 加密存储，不落明文 |
| 账号前缀 | `,0,` | 深澜协议常见要求，按实际抓包填 |
| 账号后缀 | `@unicom` | 运营商标识，按实际填 |
| 登录接口地址 | 你学校的门户地址 | 形如 `http://<门户IP>:801/eportal/portal/login` |
| 网卡关键字 | `WLAN` | 用于挑选正确的内网 IP |
| 检查间隔 | `30` 秒 | 未联网时的重试间隔 |

界面会实时显示拼装预览（如 **「提交为：,0,你的学号@unicom」**），确认格式正确。
...
| 账号处理 | 原样提交，需手填完整账号 | **自动拼前缀/后缀，只填裸账号** |
| 成功判定 | 页面关键词匹配 | **解析 `result` 字段**，精确 |
| 版本兼容 | 逻辑与移动模板耦合 | **全配置化，可移植到任意深澜学校** |
| 表单识别 | 正则扒 form（对接口无效） | 移除（不适用） |
| 日志 | — | 记录结果但**不记录账号密码** |
